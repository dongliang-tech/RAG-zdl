...
class JinaReranker:
    ...
    def rerank_documents(self, query: str, documents: list, top_n: int = 6, **kwargs) -> list:
        """
        使用 Jina Reranker 对文档进行重排。
        
        参数：
            query: 查询文本
            documents: 文档列表，每个文档包含 'text' 字段
            top_n: 返回的顶部结果数量
            **kwargs: 额外参数（用于兼容LLMReranker接口，Jina Reranker会忽略这些参数）
        
        返回：
            重排后的文档列表，每个文档包含 'text', 'score', 'page' 等字段
        """
        # 忽略 LLMReranker 特有的参数（documents_batch_size, llm_weight）
        # 这些参数对 Jina Reranker 无意义，但为了接口兼容性需要接受
...
class LLMReranker:
    ...
    def get_rank_for_multiple_blocks(self, query, retrieved_documents):
        ...
        if self.provider == "openai":
            ...
            #print('rsp=', rsp)
            # 使用get方法防止空值错误
            if rsp.get('output') and 'choices' in rsp['output']:
                content = rsp['output']['choices'][0]['message']['content']
                # 这里只返回字符串，后续可按需解析
