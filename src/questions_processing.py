...
class QuestionsProcessor:
    def __init__(
        ...
        answering_model: str = "Qwen3.6-plus", # gpt-4o-2024-08-06
    ...
    def get_answer_for_company_streaming(self, company_name: str, question: str, schema: str):
        """
        流式获取单个公司的答案，yield 以下事件:
        - {"event": "retrieval_done"} - 检索完成
        - {"event": "streaming_token", "token": str, "accumulated": str} - LLM生成的token
        - {"event": "step_by_step", "content": str} - 解析后的分步推理
        - {"event": "reasoning_summary", "content": str} - 解析后的推理摘要
        - {"event": "relevant_pages", "content": list} - 解析后的相关页面
        ...
        try:
            ...
            if not retrieval_results:
                yield {"event": "error", "error": "No relevant context found"}
                return
            
            rag_context = self._format_retrieval_results(retrieval_results)
            yield {"event": "retrieval_done"}
            
            streaming_result = self.openai_processor.get_answer_streaming(
                question=question,
                rag_context=rag_context,
                schema=schema,
                model=self.answering_model
            ...
            for chunk in streaming_result:
                if chunk.get("stage") == "streaming":
                    yield {"event": "streaming_token", "token": chunk["token"], "accumulated": chunk["accumulated"]}
                elif chunk.get("stage") == "done":
                    answer_dict = chunk["answer_dict"]
                    ...
                    yield {"event": "step_by_step", "content": step_by_step}
                    yield {"event": "reasoning_summary", "content": reasoning_summary}
                    yield {"event": "relevant_pages", "content": relevant_pages}
                    yield {"event": "final_answer", "content": final_answer}
                    yield {"event": "complete", "answer_dict": answer_dict}
                    return
                elif chunk.get("stage") == "error":
                    yield {"event": "error", "error": chunk["error"]}
                    return
    ...
    def process_single_question(self, question: str, kind: str = "string", company_name: str = None):
        """
        单条问题推理，返回结构化答案。
        kind: 支持 'string'、'number'、'boolean'、'names' 等
        company_name: 可选，直接指定公司名，避免从问题中抽取
        """
        t0 = time.time()
        print("[计时] [单问] 开始公司名抽取 ...")
        # 公司名抽取
