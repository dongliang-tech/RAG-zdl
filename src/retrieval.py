...
class HybridRetriever:
    def __init__(self, vector_db_dir: Path, documents_dir: Path, reranker_type: str = "jina"):
        """
        初始化混合检索器
        ...
        参数：
            ...
            documents_dir: 文档目录
            reranker_type: 重排器类型，'jina'（快速）或 'llm'（高质量但慢）
        """
        self.vector_retriever = VectorRetriever(vector_db_dir, documents_dir)
        if reranker_type.lower() == "jina":
            self.reranker = JinaReranker()
            self.reranker_name = "Jina Reranker"
        else:
            self.reranker = LLMReranker(provider="openai")
            self.reranker_name = "LLM Reranker"
    ...
    def retrieve_by_company_name(
        ...
        # 使用重排器对结果进行重排
        print(f"[计时] [HybridRetriever] 开始{self.reranker_name}重排 ...")
        ...
        t2 = time.time()
        print(f"[计时] [HybridRetriever] {self.reranker_name}重排耗时: {t2-t1:.2f} 秒")
