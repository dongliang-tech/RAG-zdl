...
class RunConfig:
    ...
    answering_model: str = "Qwen3.6-plus" # qwen-turbo, deepseek-v4-pro
    config_suffix: str = ""
...
class Pipeline:
    ...
    def answer_single_question(self, question: str, kind: str = "string", company_name: str = None):
        """
        单条问题即时推理，返回结构化答案（dict）。
        kind: 支持 'string'、'number'、'boolean'、'names' 等
        company_name: 可选，直接指定公司名
        """
        t0 = time.time()
        ...
        answer = processor.process_single_question(question, kind=kind, company_name=company_name)
        t2 = time.time()
        print(f"[计时] process_single_question 推理耗时: {t2-t1:.2f} 秒")
        print(f"[计时] answer_single_question 总耗时: {t2-t0:.2f} 秒")
        return answer

    def answer_single_question_streaming(self, question: str, kind: str = "string", company_name: str = None):
        """
        单条问题流式推理，yield 以下事件:
        - {"event": "retrieval_done"} - 检索完成
        - {"event": "streaming_token", "token": str, "accumulated": str} - LLM生成的token
        - {"event": "step_by_step", "content": str} - 分步推理
        - {"event": "reasoning_summary", "content": str} - 推理摘要
        - {"event": "relevant_pages", "content": list} - 相关页面
        - {"event": "final_answer", "content": str} - 最终答案
        - {"event": "complete"} - 流式完成
        - {"event": "error", "error": str} - 错误信息
        """
        t0 = time.time()
        print("[计时] [streaming] 开始初始化 QuestionsProcessor ...")
        processor = QuestionsProcessor(
            vector_db_dir=self.paths.vector_db_dir,
            documents_dir=self.paths.documents_dir,
        ...
        t1 = time.time()
        print(f"[计时] [streaming] QuestionsProcessor 初始化耗时: {t1-t0:.2f} 秒")
        print("[计时] [streaming] 开始调用流式问题处理 ...")
        
        # 公司名抽取
        if company_name:
            extracted_companies = [company_name]
        elif processor.new_challenge_pipeline:
        ...
        if len(extracted_companies) == 0:
            yield {"event": "error", "error": "No company name found in the question."}
            return
        
        if len(extracted_companies) == 1:
            company_name = extracted_companies[0]
            print("[计时] [streaming] 开始流式检索与LLM推理 ...")
            t2 = time.time()
            for event in processor.get_answer_for_company_streaming(company_name=company_name, question=question, schema=kind):
                yield event
            t3 = time.time()
            print(f"[计时] [streaming] 流式检索+LLM推理耗时: {t3-t2:.2f} 秒")
            print(f"[计时] [streaming] 流式总耗时: {t3-t0:.2f} 秒")
        else:
            yield {"event": "error", "error": "Streaming only supports single company questions. For multi-company comparison, please use the non-streaming mode."}
            return
